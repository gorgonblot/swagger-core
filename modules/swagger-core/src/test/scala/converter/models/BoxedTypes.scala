...
case class BoxedTypesIssue31(stringSeq: Seq[String], stringOpt: Option[String],
                             @(ApiModelProperty @field)(value = "Integers in a Sequence Box", required = false) intSeq: Seq[Int],
...
case class BoxedTypesIssue31WithDataType(stringSeq: Seq[String], stringOpt: Option[String],
                                         @(ApiModelProperty @field)(value = "Integers in a Sequence Box", dataType = "List[int]", required = false) intSeq: Seq[Int],
...
case class ListReply[T](
                         @(ApiModelProperty @field)(value = "List of requested items (in current page if paged)", required = false) items: List[T],
                         @(ApiModelProperty @field)(value = "Total number of items in the reply (across all pages if applicable)", required = false) total: Int,
                         @(ApiModelProperty @field)(value = "Identifier used to fetch the next page of results") nextPageToken: Option[String],
                         @(ApiModelProperty @field)(value = "Offset within the total count of results where this current items list starts", required = false) offset: Int,
